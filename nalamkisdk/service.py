...
class NaLamKIService:
    ...
    def save_blueprint(self):
        ...
        if "dashboard_blueprint.json" in os.listdir("config"):
            shutil.copy(
                os.path.join("config", "dashboard_blueprint.json"),
                os.path.join(path, "dashboard_blueprint.json"),
            )
        elif "dashboard_template.json" in os.listdir("config"):
            shutil.copy(
                os.path.join("config", "dashboard_template.json"),
                os.path.join(path, "dashboard_blueprint.json"),
            )
        else:
    ...
    def json_to_string(self, json_data):
        ...
        """
        return json.dumps(
            dataclasses.asdict(json_data), cls=NaLamKIDataEncoder, indent=2
        )
    ...
    def on_message(self, ch, method, properties, body):
        ...
        try:
            ...
            traceback.print_exc()
            print(body)
            self.rmq.write_message(
                json.dumps(
                    dataclasses.asdict(Action(pattern="ERROR", data=None)),
    ...
    def get_action_data(self, action: Action):
        ...
        self.s3 = MinIOHelper(
            bucket.endpoint, bucket.accessId, bucket.accessToken, bucket.name
        )
        ...
        for inputFile in action.data.inputData:
            if self.s3.is_directory(inputFile):
                files.extend(
                    self.s3.list_file_object_names(recursive=True, prefix=inputFile)
                )
        ...
        for file in files:
            self.s3.download_File(
                file, os.path.join(self.action_path, "input", os.path.basename(file))
            )
    ...
    def upload_action_data(self, action: Action):
        ...
        for file in os.listdir(path):
            self.s3.upload_File(
                ...
                object_file=os.path.join(action.data.outputData, file),
            )
    ...
    def send_finish_message(self, action: Action):
        ...
        self.rmq.write_message(
            json.dumps(dataclasses.asdict(action), cls=NaLamKIDataEncoder)
        )
